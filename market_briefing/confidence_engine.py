...
# 로컬 추론(무료 · API 키 불필요) — transformers 설치 시 자동 사용.
#   미설치/로드 실패 시 None → 상위에서 HF Inference API(토큰 있을 때) → 키워드로 폴백.
try:
...
# 로컬(오프라인) 모델 — 영어: FinBERT, 한국어: KR-FinBert-SC. 공개 모델이라 토큰 불필요.
_LOCAL_MODEL_EN = "ProsusAI/finbert"
_LOCAL_MODEL_KR = "snunlp/KR-FinBert-SC"
_LOCAL_PIPE: Dict[str, Any] = {}        # lang -> pipeline | False(로드 실패)
_RECENCY_HALF_LIFE_H = 48.0


def _local_finbert(texts: List[str], lang: str) -> Optional[List]:
    """로컬 transformers 파이프라인 추론 (무료 · API 키 불필요 · 미국+한국 모두 지원).

    · 영어(en) → ProsusAI/finbert,  한국어(ko) → snunlp/KR-FinBert-SC
    · 모델은 언어별 1회 로드 후 모듈 전역(_LOCAL_PIPE)에 캐시(웜 인스턴스 재사용).
    · transformers 미설치/모델 로드/추론 실패 시 None → HF API 또는 키워드로 폴백.

    반환 형식은 HF Inference API와 동일: List[List[{"label","score"}]] → _parse_finbert 재사용 가능.
    """
    if _hf_pipeline is None or not texts:
        return None
    pipe = _LOCAL_PIPE.get(lang)
    if pipe is False:                    # 직전 로드 실패 → 재시도 안 함
        return None
    if pipe is None:
        model = _LOCAL_MODEL_KR if lang == "ko" else _LOCAL_MODEL_EN
        try:
            pipe = _hf_pipeline(
                "text-classification", model=model,
                top_k=None, truncation=True, max_length=512,
            )
            _LOCAL_PIPE[lang] = pipe
            logger.info(f"Loaded local sentiment model: {model}")
        except Exception as e:
            logger.warning(f"Local sentiment model load failed ({lang}): {e}")
            _LOCAL_PIPE[lang] = False
            return None
    try:
        out = pipe(list(texts))
        # top_k=None: 배치 입력이면 List[List[dict]]. 단일이 평탄화된 경우 방어적 래핑.
        if out and isinstance(out[0], dict):
            out = [out]
        return out
    except Exception as e:
        logger.warning(f"Local sentiment inference failed ({lang}): {e}")
        return None
...
# ── 무료 규칙기반(금융사전) 보강 요소 — transformers/torch·API 불필요 (Vercel 호환) ──
# 강도(intensifier) / 완화(diminisher) / 부정(negator) 수식어
_EN_INTENSIFIERS = {"strongly", "sharply", "sharp", "surge", "soar", "massive", "record",
                    "huge", "significantly", "substantially", "steep", "skyrocket"}
_EN_DIMINISHERS = {"slightly", "marginally", "modest", "slight", "somewhat", "mild", "tad"}
_EN_NEGATORS = {"no", "not", "never", "without", "fails", "failed", "lacks", "lack",
                "unlikely", "denies", "denied", "avoid", "avoids", "isn't", "aren't",
                "wasn't", "don't", "doesn't", "didn't", "cannot"}
_KR_INTENS = ["급", "대폭", "크게", "사상", "역대", "최대", "강하게", "대규모", "훌쩍", "폭"]
_KR_DIMIN = ["소폭", "약간", "다소", "소규모", "미미", "제한적"]
# 악재어 뒤에 오면 의미를 뒤집는 '해소/완화' 류 (예: "우려 해소", "리스크 완화")
_KR_FLIP_AFTER = ["해소", "완화", "불식", "벗어", "진정", "안도"]

# 구(phrase) 단위 신호 — 단어보다 강함(가중↑). 영어는 소문자, 한국어는 원문 매칭.
_PHRASES_POS = {
    "beats estimates": 2.0, "earnings beat": 2.0, "record high": 2.0, "all-time high": 2.2,
    "raises guidance": 2.0, "better than expected": 2.0, "strong demand": 1.5, "buy rating": 1.5,
    "price target raised": 1.8, "사상 최대": 2.0, "어닝 서프라이즈": 2.2, "실적 호조": 2.0,
    "신고가 경신": 1.8, "목표가 상향": 1.8, "흑자 전환": 2.0, "흑자전환": 2.0,
    "자사주 매입": 1.5, "배당 확대": 1.4, "대규모 수주": 1.8,
}
_PHRASES_NEG = {
    "misses estimates": 2.0, "profit warning": 2.2, "cuts guidance": 2.0, "guidance cut": 2.0,
    "worse than expected": 2.0, "below expectations": 1.8, "files for bankruptcy": 2.5,
    "sell rating": 1.5, "price target cut": 1.8, "어닝 쇼크": 2.2, "실적 쇼크": 2.2,
    "적자 전환": 2.0, "적자전환": 2.0, "목표가 하향": 1.8, "신저가": 1.6, "유상증자": 1.2,
    "분식회계": 2.5, "상장폐지": 2.5, "거래정지": 2.0,
}
...
def _kr_word_signal(raw: str, idx: int, wlen: int, base: float) -> float:
    """한국어 감정어 1건의 부호·강도 — 인접 강조/완화/부정(해소·완화) 반영.

    base: +1(호재어) / -1(악재어). 반환값 부호가 base와 뒤집힐 수 있음(예: "우려 해소").
    """
    before = raw[max(0, idx - 4):idx]
    after = raw[idx + wlen: idx + wlen + 6]
    val = base
    if any(k in before for k in _KR_INTENS):
        val *= 1.5
    if any(k in before for k in _KR_DIMIN):
        val *= 0.6
    # 악재어 뒤 '해소/완화' → 호재로 반전 (호재어 뒤엔 거의 안 옴)
    if base < 0 and any(k in after for k in _KR_FLIP_AFTER):
        val = abs(val) * 0.8
    return val


def _keyword_sentiment(text: str) -> Dict[str, float]:
    """규칙기반(금융사전) 감정 분석 — 무료·무의존성(Vercel 호환).

    개선점(단순 키워드 카운트 대비):
      · 구(phrase) 단위 신호(가중↑) · 등락 기호/±N% 수치 신호
      · 영어 부정(negator) 윈도우 + 강도/완화 승수
      · 한국어 강조/완화 + '해소·완화' 반전 처리
    """
    raw = text or ""
    low = raw.lower()
    pos = neg = 0.0
    ...
    # ① 구 단위 (영어 소문자 / 한국어 원문)
    for ph, w in _PHRASES_POS.items():
        if ph in low or ph in raw:
            pos += w
    for ph, w in _PHRASES_NEG.items():
        if ph in low or ph in raw:
            neg += w
    ...
    for m in re.finditer(r"([+\-])\s?(\d+(?:\.\d+)?)\s?%", raw):
        mag = min(1.2, 0.3 + float(m.group(2)) / 15.0)
        if m.group(1) == "+":
            pos += mag
        else:
            neg += mag

    # ③ 한국어 단어 (강조/완화/반전 인접 처리)
    if _has_hangul(raw):
        for w in _KR_BULLISH:
            for idx in _find_all(raw, w):
                s = _kr_word_signal(raw, idx, len(w), +1.0)
                pos += s if s >= 0 else 0.0
                neg += -s if s < 0 else 0.0
        for w in _KR_BEARISH:
            for idx in _find_all(raw, w):
                s = _kr_word_signal(raw, idx, len(w), -1.0)
                neg += -s if s <= 0 else 0.0
                pos += s if s > 0 else 0.0   # '해소/완화'로 반전된 경우

    # ④ 영어 토큰 (부정 윈도우 3 + 강도/완화 승수)
    toks = re.split(r"\W+", low)
    for i, t in enumerate(toks):
        base = 1.0 if t in _BULLISH else -1.0 if t in _BEARISH else 0.0
        if base == 0.0:
            continue
        mult, flip = 1.0, False
        for j in range(max(0, i - 3), i):
            if toks[j] in _EN_NEGATORS:
                flip = not flip
            if toks[j] in _EN_INTENSIFIERS:
                mult *= 1.5
            if toks[j] in _EN_DIMINISHERS:
                mult *= 0.6
        val = base * mult * (-1.0 if flip else 1.0)
        pos += val if val >= 0 else 0.0
        neg += -val if val < 0 else 0.0

    score = _clamp((pos - neg) / 3.0, -1.0, 1.0)
...
def analyze_news_sentiment(news_items: List[Dict]) -> Dict[str, Any]:
    ...
    # 추론 우선순위: ① 로컬 모델(무료·키 불필요·US+KR) → ② HF Inference API(토큰 있을 때) → ③ 키워드
    finbert_raw = _local_finbert(titles, lang)
    if finbert_raw is None:
    ...
    eng = {"finbert": "FinBERT", "kr-finbert": "KR-FinBERT", "keyword": "금융사전(규칙기반)"}.get(source, source)
    if overall == "positive":
