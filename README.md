...
순수 Python으로 구현되어 TA-Lib 없이 26개 캔들스틱 패턴을 인식합니다.
...
| **21개 기술적 지표** | MA, EMA, RSI, MACD, 볼린저밴드, ATR, ADX, Stochastic 등 |
| **가중치 기반 종합 점수** | 추세·모멘텀·변동성·거래량·캔들 패턴 5개 축 0~100 점수화 |
...
| 분류 | 지표 |
|---|---|
| **추세** | MA5, MA20, MA60, MA120, EMA12, EMA13, EMA20, EMA26, EMA50 |
| **모멘텀** | RSI(14), MACD, Signal Line, ROC(12), Stochastic %K/%D, StochRSI |
| **변동성** | Bollinger Bands(20,2), ATR(14), Williams %R |
| **추세 강도** | ADX(14), DI+, DI−, Bull/Bear Power, Ultimate Oscillator |

---

## 🧮 종합 점수 시스템

Base 50점에서 각 축의 가중치만큼 가감하여 0~100점으로 산출합니다.

| 축 | 가중치 | 주요 지표 |
|---|---|---|
| 추세 분석 | 35% | EMA20/50 정배열, 현재가 vs EMA20, MACD 크로스 |
| 모멘텀 | 30% | RSI(14) 과매수/과매도, ADX(14) + DI 방향 |
| 변동성 | 20% | 볼린저밴드 위치, ATR 대비 변동 비율 |
| 거래량 | 15% | 현재 거래량 vs 20일 평균 |
| 캔들 패턴 | 보조 | 상승 패턴 +2점, 하락 패턴 −2점 (최대 ±5) |
